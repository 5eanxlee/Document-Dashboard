...
Use DocDash as the system of record for registered documents. Prefer the `docdash` MCP server from `.mcp.json`; when that is unavailable, use `npm run docdash ...` from the repo root. For global Codex access from any directory, install the repo into Codex once with `npm run codex:install`.
...
1. Prefer the project `docdash` MCP server.
2. Fall back to `npm run docdash ...` commands.
...
- Use `add_doc` or `npm run docdash add <file>`.
...
Read [references/interfaces.md](references/interfaces.md) when you need the exact CLI commands, global Codex install steps, MCP tool names, or the repo-specific Codex/Claude/Hermes entry points.
